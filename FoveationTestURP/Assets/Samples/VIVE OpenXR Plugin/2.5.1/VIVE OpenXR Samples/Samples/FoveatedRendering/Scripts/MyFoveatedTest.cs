...
public class MyFoveatedTest : MonoBehaviour
{
   ...
   // public static XrFoveationConfigurationHTC[] configs = { config_left, config_right };
    ...
    private XrFoveationConfigurationHTC[] configs;

    void Start()
    {
	    configs = new XrFoveationConfigurationHTC[2];
	    configs[0] = new XrFoveationConfigurationHTC();
	    configs[1] = new XrFoveationConfigurationHTC();

	    ///////////////////// Extreme Foveation for Left Eye /////////////////////////////
	    configs[0].level = XrFoveationLevelHTC.XR_FOVEATION_LEVEL_HIGH_HTC;
	    configs[0].clearFovDegree = 5.0f;  // Very narrow high-quality center
	    configs[0].focalCenterOffset.x = 0.0f;
	    configs[0].focalCenterOffset.y = -0.1f;

	    //////////////////// Extreme Foveation for Right Eye /////////////////////////////
	    configs[1].level = XrFoveationLevelHTC.XR_FOVEATION_LEVEL_HIGH_HTC;
	    configs[1].clearFovDegree = 5.0f;
	    configs[1].focalCenterOffset.x = 0.0f;
	    configs[1].focalCenterOffset.y = -0.1f;

	    //////////////////// Apply Custom Foveation /////////////////////////////
	    ViveFoveation.ApplyFoveationHTC(XrFoveationModeHTC.XR_FOVEATION_MODE_CUSTOM_HTC, 2, configs);

	    Debug.Log("Aggressive foveation applied.");
    }
